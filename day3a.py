...
def parse_line(l):
    result = [(token[0], int(token[1:])) for token in l.split(",")]
    return result
...
if __name__ == "__main__":
    ...
    intersected = traversed_a.intersection(traversed_b)
    print("Answer:", min(sum(abs(i) for i in location) for location in intersected))
